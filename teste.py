...
BoxLayout:
    ...
    GridLayout:
        ...
        MDRaisedButton:
            text: "/"
            on_release: app.on_operator_press("/")
...
class CalculadoraApp(BoxLayout):
    ...
    def on_number_press(self, number):  # Corrigido: on_number_press (não on_numver_press)
        current_text = self.ids.input_field.text
        new_text = f"{current_text}{number}"
        self.ids.input_field.text = new_text

    def on_operator_press(self, operator):  # Corrigido: on_operator_press
        current_text = self.ids.input_field.text
        new_text = f"{current_text}{operator}"
        self.ids.input_field.text = new_text

    def clear_input(self):  # Corrigido: clear_input (não clear_inpur)
        self.ids.input_field.text = ""

    def calculate_result(self):  # Corrigido: calculate_result
        try:
            result = eval(self.ids.input_field.text)
            self.ids.input_field.text = str(result)  # Corrigido: input_field (não input_filed)
        except Exception as e:
            self.ids.input_field.text = "Erro"

class CalculatorMDApp(MDApp):
    def build(self):
        return CalculadoraApp()  # Corrigido: CalculadoraApp (não CalculadoresApp)
    
    def on_number_press(self, number):
        self.root.on_number_press(number)
    
    def on_operator_press(self, operator):  # Adicionado método faltante
        self.root.on_operator_press(operator)
    
    def clear_input(self):
        self.root.clear_input()
    
    def calculate_result(self):  # Corrigido: calculate_result (não calculator_result)
        self.root.calculate_result()  # Corrigido: self.root (não self.room)

if __name__ == "__main__":
    Builder.load_string(kv_string)  # Correto: apenas a string
    CalculatorMDApp().run()